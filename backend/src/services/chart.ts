...
export async function buildChart(args: ChartArgs): Promise<{ imageBase64?: string; meta: any }> {
  ...
  // Parse the flattened context into plant-centric rows
  type Row = { plant: string; accepted?: number; rejected?: number; actual_readings?: number };
  const rowsMap = new Map<string, Row>();
  let currentPlant = "";
  const lines = args.context.split(/\n|,|;|\|/g).map((s) => s.trim()).filter(Boolean);
  const ensureRow = (plant: string) => {
    if (!rowsMap.has(plant)) rowsMap.set(plant, { plant });
  ...
  for (const line of lines) {
    ...
    if (mName) { currentPlant = mName[1].trim(); ensureRow(currentPlant); continue; }
    if (mId) { if (!currentPlant) currentPlant = mId[1].trim(); ensureRow(currentPlant); continue; }

    // metrics
    const mAcc = line.match(/accepted\s*:\s*(-?\d+(?:\.\d+)?)/i);
    if (mAcc && currentPlant) { const r = ensureRow(currentPlant); r.accepted = Number(mAcc[1]); continue; }
    const mRej = line.match(/rejected\s*:\s*(-?\d+(?:\.\d+)?)/i);
    if (mRej && currentPlant) { const r = ensureRow(currentPlant); r.rejected = Number(mRej[1]); continue; }
    const mRead = line.match(/actual[_\s]?readings?\s*:\s*(-?\d+(?:\.\d+)?)/i);
    if (mRead && currentPlant) { const r = ensureRow(currentPlant); r.actual_readings = Number(mRead[1]); continue; }
  }
  ...
  if (rows.length === 0) {
    ...
    if (args.output === "json") return { meta: { type, width, height, spec: config } };
  ...
  const has = (k: keyof Row) => rows.some((r) => typeof r[k] === "number");
  const labels = rows.map((r) => r.plant || "(unknown)");
  const color = (i: number, a = 0.6) => `hsla(${(i * 53) % 360},70%,60%,${a})`;
  ...
  if (type === "pie" || type === "doughnut") {
    ...
    let metric: keyof Row = "accepted";
    const mentionsAccepted = /accepted/.test(promptLower);
    const mentionsRejected = /rejected/.test(promptLower);
    if (mentionsRejected && !mentionsAccepted) metric = "rejected";
    else if (/actual[_\s]?readings?/.test(promptLower)) metric = "actual_readings";
    ...
    const datasets: any[] = [];
    if (has("accepted")) datasets.push({ label: "accepted", data: rows.map((r) => r.accepted ?? 0), backgroundColor: "rgba(54,162,235,0.5)", borderColor: "rgba(54,162,235,1)", borderWidth: 1 });
    if (has("rejected")) datasets.push({ label: "rejected", data: rows.map((r) => r.rejected ?? 0), backgroundColor: "rgba(255,99,132,0.5)", borderColor: "rgba(255,99,132,1)", borderWidth: 1 });
    if (has("actual_readings")) datasets.push({ label: "actual_readings", data: rows.map((r) => r.actual_readings ?? 0), backgroundColor: "rgba(75,192,192,0.5)", borderColor: "rgba(75,192,192,1)", borderWidth: 1 });
    if (datasets.length === 0) datasets.push({ label: "value", data: rows.map(() => 0), backgroundColor: "rgba(180,180,180,0.5)", borderColor: "rgba(180,180,180,1)", borderWidth: 1 });
