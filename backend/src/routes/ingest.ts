...
const QCDataSchema = z.object({
  namespace: z.string().optional().default("qc_inspections"),
  data: z.array(z.record(z.string(), z.any())).min(1),
  chunkSize: z.number().int().min(200).max(2000).optional().default(1000),
  chunkOverlap: z.number().int().min(50).max(300).optional().default(150),
});

ingestRouter.post("/qc-data", async (req, res) => {
  try {
