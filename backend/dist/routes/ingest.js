...
exports.ingestRouter.post("/", async (req, res) => {
    try {
        ...
        await adaptiveVectorStore_1.adaptiveVectorStore.upsert(allChunks.map((d, i) => ({
            id: d.id,
            text: d.text,
            metadata: d.metadata ?? {},
            embedding: vectors[i],
        ...
        res.json({
            ...
            namespace: namespace ?? "default",
            // Special endpoint for QC inspection data
            const: QCDataSchema = zod_1.z.object({
                namespace: zod_1.z.string().optional().default("qc_inspections"),
                data: zod_1.z.array(zod_1.z.record(zod_1.z.string(), zod_1.z.any())).min(1),
                chunkSize: zod_1.z.number().int().min(200).max(2000).optional().default(1000),
                chunkOverlap: zod_1.z.number().int().min(50).max(300).optional().default(150),
            }),
            ingestRouter: exports.ingestRouter, : .post("/qc-data", async (req, res) => {
                try {
                    const { namespace, data, chunkSize, chunkOverlap } = QCDataSchema.parse(req.body);
                    console.log(`Processing ${data.length} QC inspection records...`);
                    const allChunks = [];
                    ...
                    for (let i = 0; i < allChunks.length; i += batchSize) {
                        ...
                        const vectors = await embeddings_1.embeddings.embedMany(texts);
                        await adaptiveVectorStore_1.adaptiveVectorStore.upsert(batch.map((d, idx) => ({
                            id: d.id,
                            text: d.text,
                            metadata: d.metadata ?? {},
                    ...
                    console.error("QC data ingestion error:", err);
                    res.status(400).json({ ok: false, error: err.message ?? "QC data ingestion failed" });
                }
            }),
            ...
            ingestRouter: exports.ingestRouter, : .post("/load-qc", async (req, res) => {
                try {
